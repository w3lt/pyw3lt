...
pub async fn search_packages(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        results
            .into_iter()
            .map(|(name, _score)| query_package_info(name.to_string(), project_path.clone()))
...
pub async fn sync_package_list() -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || utils::pypi::sync_package_list().unwrap())
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn get_last_sync_time() -> Result<usize, String> {
    tauri::async_runtime::spawn_blocking(move || utils::pypi::get_last_sync_time().unwrap())
        .await
        .map_err(|e| e.to_string())
}
